...
git commit
   ...
   │      · 代码超集 / Spec 超集 → 提醒
   │
   ├─ ⑤ AGENTS.md 行数守卫        —— >115 行触发迁出提醒
   │
   └─ ⑥ 上下文缝补检查            —— 本次是否外置了逻辑而未缝补
```
...
本模板**不预置 hooks 脚本**（保持语言无关、零代码）。
因此「强制是真强制」由三条机制共同构成 —— **缺任何一条都会退化成靠自觉**：

| 机制 | 作用 | 落点 |
|---|---|---|
| **硬指令** | 每次会话必加载，写明「缺一项不得提交」 | `AGENTS.md` 第 8 节 |
| **逐项留痕** | 勾选结果附进 commit message，**能被人看到的检查才会真的发生** | [commit-checklist.md](commit-checklist.md) |
| **月度审计** | 被绕过的提交集中列出，反推关卡设计问题 | changelog 月度回顾 |

**诚实的局限**：这是「强约束的行为协议」，不是技术拦截。
它能挡住「忘了做」，挡不住「铁了心绕过」。
...
本模板**不预置可执行脚本**（保持语言无关）。按你的技术栈选择装配方式，职责不变：
...
| 钩子 | 放什么 | 原因 |
|---|---|---|
| `pre-commit` | ① 质量门禁（lint/typecheck/test） | 快、可拦截 |
| `commit-msg` | 规范 commit message（便于 changelog 提取） | 需要 message 内容 |
| `post-commit` | ②③⑤ 知识沉淀与守卫 | 提交已完成，不阻塞开发 |
...
`collar.yaml` 的 `validation.gates` 声明了哪些门禁是必须的、
