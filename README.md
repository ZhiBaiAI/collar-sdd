...
> **一句话**：给 AI Agent 套「项圈」的项目脚手架 —— 对 Agent 来说，看不到的知识等于不存在，
> Collar 把项目上下文**结构化、版本化、自动化**地写进仓库文件，AI 不再每会话重灌上下文。
>
> - **是什么**：`AGENTS.md` 入口地图 + 六模块结构化知识库（specs/changelog/architecture/runbook/vendor/wiki）
>   + 六个配套 Skill + `collar.yaml` 边界与门禁声明 + commit 统一关卡
> - **解决什么**：AI 编码在真实项目里"局部正确、整体错误"——根因是上下文缺失与断裂，不是模型不够聪明
> - **怎么用**：复制本仓库到新项目根，按「快速开始」7 步冷启，约 15 分钟
> - **语言无关**：不绑定技术栈，不绑定任何 AI 工具（Cursor / Claude Code / WorkBuddy / 任意 Agent）
...
| ✂ 冷启后删 | 本文件的「快速开始」「落地清理清单」「随团队成长逐步启用」三节（模板使用说明，冷启后即失效）。**本文件可整体重写为项目说明**：其中的使用纪律与裁剪原则已双写进运行时文件（`collar.yaml` gates / `conventions.md` / `skills/README.md`），此处只是导读副本，改掉 README 不影响任何规则 |
